...
hh_e_upgrades hh_shop(hh_e_game_state* hh_game_state, hh_g_all_levels* levels, int rng_seed, int points){
	...
	case hh_e_shop_show:
		...
		char* c[3];
		hh_itoa(c,levels->current_level);
		hh_ui_show_char(&idx,c,(vec2){304-16-8,0});

		static char* player_points = NULL; // declare player_points as a static variable to retain its value between function calls

		char buffer[20];
		sprintf(buffer, "%d", points);
		int len = strlen(buffer);
		char* new_player_points = malloc(len + 1);
		strcpy(new_player_points, buffer); 
		if (player_points != NULL) {
			free(player_points); 
		}
		player_points = new_player_points; 
		hh_ui_show_char(&idx, player_points, (vec2){0,0});
